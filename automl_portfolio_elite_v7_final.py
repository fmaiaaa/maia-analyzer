...
def carregar_dados_ativo_gcs_csv(base_url: str, ticker: str, file_suffix: str) -> pd.DataFrame:
    """
    [CORRIGIDO V8] Carrega o DataFrame de um único ativo (ticker) com sufixo específico 
    via URL pública do GCS (formato CSV).

    Args:
        base_url (str): A URL base do bucket GCS (Ex: https://storage.googleapis.com/bucket-name/folder/).
        ticker (str): O código do ativo (Ex: 'PETR4.SA').
        file_suffix (str): O sufixo do arquivo (Ex: '_tecnicos.csv', '_fundamentos.csv').

    Returns:
        pd.DataFrame: O DataFrame carregado e formatado, ou um DataFrame vazio em caso de erro.
    """
    
    # Constrói o nome completo do arquivo e a URL
    file_name = f"{ticker}{file_suffix}" 
    full_url = f"{base_url}{file_name}"
    
    try:
        # Tenta carregar o arquivo CSV via URL HTTP
        df_ativo = pd.read_csv(full_url)
        
        # 1. Ajuste do Índice de Data
        # Verifica se o índice foi salvo como 'Date' ou 'index'
        ...
        # Converte o índice para datetime e remove a informação de fuso horário
        ...
        # Garante que colunas numéricas (exceto strings de identificação) sejam float (trata erros como NaN)
...
class ConstrutorPortfolioAutoML:
    ...
    def coletar_e_processar_dados(self, simbolos: list) -> bool:
        ...
        # NOTE: A classe ColetorDadosGCS deve estar definida e acessível
    ...
    def calcular_volatilidades_garch(self):
        """[MODIFICADO] Apenas valida se as volatilidades foram carregadas do ColetorDadosGCS."""
        ...
        if valid_vols == 0:
             ...
             for ativo in self.ativos_sucesso:
                 if ativo in self.dados_por_ativo and 'annual_volatility' in self.dados_por_ativo[ativo].columns:
                      self.volatilidades_garch[ativo] = self.dados_por_ativo[ativo]['annual_volatility'].iloc[-1]
        
    def treinar_modelos_ensemble(self, dias_lookback_ml: int = LOOKBACK_ML, otimizar: bool = False):
        """
        [MODIFICADO/PLACEHOLDER] Este método não realiza mais o treinamento ML complexo.
        Assume-se que os dados ML (ML_Proba, ML_Confidence) foram carregados pelo GCS.
        """
        pass
        
    def pontuar_e_selecionar_ativos(self, horizonte_tempo: str):
        """
        [CORRIGIDO V8] Pontua e ranqueia ativos acessando o DataFrame Temporal (df_tecnicos) 
        com Fundamentos/ML anexados na última linha.
        """
        
        # 1. Pesos Adaptativos (Cálculo Mantido)
        ...
        # 3. Garantir colunas essenciais para o score
        # Se a coluna não existe no GCS, ela é tratada como NaN
        for col in ['sharpe_ratio', 'pe_ratio', 'roe', 'rsi_14', 'macd', 'ML_Proba', 'ML_Confidence', 'sector']:
        ...
        pe_score = EngenheiroFeatures._normalizar(combinado['pe_ratio'], maior_melhor=False) # Menor P/L é melhor
        roe_score = EngenheiroFeatures._normalizar(combinado['roe'], maior_melhor=True)    # Maior ROE é melhor
        scores['fundamental_score'] = (pe_score * 0.5 + roe_score * 0.5) * WEIGHT_FUND
        
        # 4.3. Score Técnico (RSI e MACD)
        rsi_norm = EngenheiroFeatures._normalizar(combinado['rsi_14'], maior_melhor=False) # Menor RSI (oversold) é melhor
        macd_norm = EngenheiroFeatures._normalizar(combinado['macd'], maior_melhor=True) # MACD positivo é melhor
    ...
    # --- Métodos de Otimização e Métricas (Mantidos/Simplificados) ---
    
    def otimizar_alocacao(self, nivel_risco: str):
        # Implementação OtimizadorPortfolioAvancado (Mantida)
        ...
        self.metodo_alocacao_atual = f'{strategy} (GARCH/Histórico)'; return self._formatar_alocacao(weights)
    ...
    def calcular_metricas_portfolio(self):
        # Implementação Métricas de Portfólio (Mantida)
    ...
    def gerar_justificativas(self):
        # Implementação Geração de Justificativas (Mantida/Ajustada)
        self.justificativas_selecao = {}
        for simbolo in self.ativos_selecionados:
            justification = []
            
            last_row = self.dados_por_ativo[simbolo].iloc[-1] if simbolo in self.dados_por_ativo else pd.Series({})

            # Performance (Sharpe, Vol, Ret)
            sharpe = last_row.get('sharpe_ratio', np.nan)
            ann_ret = last_row.get('annual_return', np.nan)
            vol_garch = last_row.get('garch_volatility', np.nan)
            justification.append(f"Perf: Sharpe {sharpe:.3f}, Retorno {ann_ret*100:.2f}%, Vol. {vol_garch*100:.2f}% (GARCH/Hist.)")
            
            # Fundamentos (P/L, ROE)
            pe_ratio = last_row.get('pe_ratio', np.nan)
            roe = last_row.get('roe', np.nan)
            justification.append(f"Fund: P/L {pe_ratio:.2f}, ROE {roe:.2f}%")
            
            # ML
            proba_up = last_row.get('ML_Proba', 0.5)
            auc_score = last_row.get('ML_Confidence', np.nan)
    ...
    def executar_pipeline(self, simbolos_customizados: list, perfil_inputs: dict, otimizar_ml: bool = False) -> bool:
        ...
        self.alocacao_portfolio = self.otimizar_alocacao(nivel_risco=nivel_risco) # Atualiza alocação aqui
...
class ConstrutorPortfolioAutoML:
    ...
    def treinar_modelos_ensemble(self, dias_lookback_ml: int = LOOKBACK_ML, otimizar: bool = False):
        """
        [MODIFICADO/SIMPLIFICADO] Placeholder para o treinamento ML. 
        
        Assume que os dados ML (ML_Proba e ML_Confidence) foram carregados na última 
        linha do self.dados_por_ativo pelo ColetorDadosGCS.
        """
        
        # O código de treinamento complexo (LightGBM, TimeSeriesSplit, Optuna) foi removido.
        
        # Apenas verifica se há dados ML carregados (que vieram do GCS)
        ativos_com_ml = [
            ativo for ativo, df in self.dados_por_ativo.items() 
            if 'ML_Proba' in df.columns and not pd.isna(df['ML_Proba'].iloc[-1])
        ]
        
        if len(ativos_com_ml) > 0:
            print(f"✓ Dados ML (Probabilidade de Alta) carregados do GCS para {len(ativos_com_ml)} ativos.")
        else:
            print("⚠️ Aviso: Nenhuma previsão ML encontrada nos dados do GCS. Scores de ML serão neutros (0.5).")
            
        # Não é necessário retornar nada, pois os dados já estão em self.dados_por_ativo

    def pontuar_e_selecionar_ativos(self, horizonte_tempo: str):
        """
        [CORRIGIDO V8] Pontua e ranqueia ativos usando sistema multi-fator (Perf, Fund, Tech, ML) 
        e diversificação. Acesso a dados é feito via self.dados_por_ativo (agora mais completo).
        """
        
        # 1. Pesos Adaptativos (Inalterados, baseado no horizonte)
        ...
        # DataFrame que agrega as últimas métricas de cada ativo
        last_metrics = {}
        for asset in self.ativos_sucesso:
            df = self.dados_por_ativo[asset]
            last_row = df.iloc[-1].to_dict()
            last_metrics[asset] = last_row
            # Adiciona métricas do ml_results (se existirem)
            last_metrics[asset]['ML_Proba'] = df.get('ML_Proba', 0.5).iloc[0] if 'ML_Proba' in df.columns else 0.5
            last_metrics[asset]['ML_Confidence'] = df.get('ML_Confidence', 0.5).iloc[0] if 'ML_Confidence' in df.columns else 0.5

        combinado = pd.DataFrame(last_metrics).T
        
        # Garantir que o Sharpe e as colunas de Fundamentos estejam nos dados
        if 'sharpe_ratio' not in combinado.columns: combinado['sharpe_ratio'] = np.nan
        if 'fund_pe_ratio' not in combinado.columns: combinado['fund_pe_ratio'] = np.nan
        if 'fund_roe' not in combinado.columns: combinado['fund_roe'] = np.nan
        if 'rsi_14' not in combinado.columns: combinado['rsi_14'] = np.nan
        if 'macd' not in combinado.columns: combinado['macd'] = np.nan
            
        # 2.1. Score de Performance (Sharpe)
        scores['performance_score'] = EngenheiroFeatures._normalizar(combinado['sharpe_ratio'], maior_melhor=True) * WEIGHT_PERF
        
        # 2.2. Score Fundamentalista (P/L e ROE)
        pe_score = EngenheiroFeatures._normalizar(combinado['fund_pe_ratio'], maior_melhor=False)
        roe_score = EngenheiroFeatures._normalizar(combinado['fund_roe'], maior_melhor=True)
        scores['fundamental_score'] = (pe_score * 0.5 + roe_score * 0.5) * WEIGHT_FUND
        
        # 2.3. Score Técnico (RSI e MACD - do df_tecnicos)
        rsi_norm = EngenheiroFeatures._normalizar(combinado['rsi_14'], maior_melhor=False) # Menor RSI (oversold) é melhor para um "buy signal"
        macd_norm = EngenheiroFeatures._normalizar(combinado['macd'], maior_melhor=True) # MACD positivo é melhor
        scores['technical_score'] = (rsi_norm * 0.5 + macd_norm * 0.5) * WEIGHT_TECH

        # 2.4. Score de Machine Learning (Proba e Confiança)
        ml_proba_norm = EngenheiroFeatures._normalizar(combinado['ML_Proba'], maior_melhor=True)
        ml_confidence_norm = EngenheiroFeatures._normalizar(combinado['ML_Confidence'], maior_melhor=True)
        scores['ml_score_weighted'] = (ml_proba_norm * 0.6 + ml_confidence_norm * 0.4) * final_ml_weight
        ...
        # 3. Seleção Final (Diversificação Setorial)
        ranked_assets = self.scores_combinados.index.tolist()
        final_portfolio = []
        ...
        for asset in ranked_assets:
            # A coluna 'sector' foi anexada na ColetorDadosGCS
            sector = combinado.loc[asset, 'sector'] if 'sector' in combinado.columns and asset in combinado.index else 'Unknown'
            
            # Lógica de Diversificação: Seleciona se o setor não foi usado ou se ainda não atingiu o número mínimo de ativos.
            if sector not in selected_sectors or len(final_portfolio) < num_assets_to_select:
...
def coletar_ativo_unico_gcs(self, ativo_selecionado: str) -> tuple[pd.DataFrame | None, dict | None]:
        """
        [CORRIGIDO V8] Coleta e retorna dados de um único ativo sob demanda (Aba 4).

        Carrega o histórico técnico e os fundamentos, e consolida os dados estáticos 
        (Fundamentos, Sharpe, ML) na última linha do DataFrame temporal para facilitar 
        a análise detalhada na interface Streamlit.
        """
        
        # 1. Carrega CSVs do ativo específico
        # NOTE: base_url e carregar_dados_ativo_gcs_csv devem estar definidos globalmente ou importados
        # GCS_BASE_URL é uma constante global
        ...
        if df_tecnicos.empty or df_fundamentos.empty or 'Close' not in df_tecnicos.columns:
            # Retorna None se os dados essenciais não puderem ser carregados
            return None, None
        
        # 2. Extrai Fundamentos e Métricas (df_fundamentos)
        # Assumindo que _get_fundamental_metrics_from_df está definido na classe ColetorDadosGCS
        fund_row = df_fundamentos.iloc[0]
        features_fund = self._get_fundamental_metrics_from_df(fund_row) # Retorna um dicionário de fundamentos/métricas
        
        # 3. Anexa dados ML (se disponíveis)
        df_ml_results = carregar_dados_ativo_gcs_csv(GCS_BASE_URL, ativo_selecionado, file_suffix='_ml_results.csv')
        
        # Anexa os dados estáticos e ML à última linha do DataFrame temporal
        if not df_tecnicos.empty:
            last_index = df_tecnicos.index[-1]
            
            # Anexa Fundamentos e Métricas
            ...
            if not df_ml_results.empty:
                ...
                # Procura a coluna de maior lookahead (longo prazo)
                ...
                df_tecnicos.loc[last_index, 'ML_Confidence'] = 0.5 # Placeholder para AUC
        
        # Retorna o DataFrame temporal enriquecido e o dicionário de fundamentos (para exibição na Aba 4)
