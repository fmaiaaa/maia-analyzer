...
Versão: 9.32.8 (Update: FIX Final Syntax Error & ML/GARCH Visibility Logic)
=============================================================================
"""
    ...
    def coletar_e_processar_dados(self, simbolos: list, check_min_ativos: bool = True) -> bool:
        ...
        for simbolo in simbolos:
            ...
            if not usando_fallback_estatico and 'returns' in df_tecnicos.columns:
                ...
                vol_anual = 0.20 # Fallback inicial
                garch_vol = 0.20 # Fallback inicial
            ...
            garch_vols[simbolo] = garch_vol # Armazena o valor GARCH calculado (ou fallback)
        ...
        if ativo_selecionado in self.dados_por_ativo:
             ...
             if not df_tec.empty and 'Close' in df_tec.columns and len(df_tec) > 60 and not df_tec['Close'].isnull().all():
                 try:
                     ...
                     # FIX 3: Remove sma_200 (muito exigente) do modelo individual para aumentar a robustez
...
class ConstrutorPortfolioAutoML:
    ...
    def treinar_modelos_ensemble(self, dias_lookback_ml: int = LOOKBACK_ML, otimizar: bool = False, progress_callback=None):
        ...
        # --- CORREÇÃO DE KEYERROR (Passo Anterior) ---
        ...
        # FIX 3: Remove sma_200 (muito exigente) do modelo principal para aumentar a robustez
        features_ml = ['rsi_14', 'macd_diff', 'vol_20d', 'momentum_10', 'sma_50',
...
def aba_introducao():
    ...
    <p style="font-size: 0.9rem;">2️⃣ <b>Scoring & Filtro:</b> Cálculo de indicadores, ranqueamento multi-fatorial e eliminação dos ativos com pior pontuação.</p>
    <p style="font-size: 0.9rem;">3️⃣ <b>Otimização Final:</b> Utilização do Princípio de Markowitz (Variância/Sharpe) e Clusterização (PCA/K-Means) para garantir a diversificação e a melhor relação Risco-Retorno. </p>
    </div>
    """, unsafe_allow_html=True)
...
def aba_analise_individual():
    ...
    # Adiciona a lógica para identificar se o builder existe e se há ML utilizável
    has_usable_ml = False
    if 'builder' in st.session_state and st.session_state.builder:
        # Reutiliza a lógica de checagem do construtor principal, se ele já rodou
        has_usable_ml = st.session_state.builder.scores_combinados['ml_score_weighted'].sum() > 0 if not st.session_state.builder.scores_combinados.empty else False
    ...
    with st.spinner(f"Analisando {ativo_selecionado} (Coleta em Tempo Real)..."):
        try:
            ...
            with tab2:
                ...
                # ALTERAÇÃO SOLICITADA: Substitui 'Cresc. Receita (5a)' por ROIC
